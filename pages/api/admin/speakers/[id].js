...
export default async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    const { full_name, email, is_internal, vignette_name } = req.body || {};
    const patch = {};
    if (full_name !== undefined) {
      const v = String(full_name || "").trim();
      if (!v) return json(res, 400, { error: "Nome é obrigatório." });
      patch.full_name = v;
    }
    if (email !== undefined) patch.email = String(email || "").trim().toLowerCase() || null;
    if (is_internal !== undefined) patch.is_internal = is_internal === false ? false : true;
    if (vignette_name !== undefined) patch.vignette_name = String(vignette_name || "").trim() || null;

    const { error } = await admin.from("speakers").update(patch).eq("id", id);
