...
export async function listProfiles() {
  ...
  // Preferred: server route that can use Service Role (bypasses RLS) while still validating the session.
  try {
    ...
    if (!r.ok) {
      ...
      throw new Error(j?.error || "Falha ao listar usuários");
    }
    const j = await r.json();
    return j?.profiles ?? [];
  } catch (e) {
    // Fallback: direct query (may be blocked by RLS depending on policies)
    const { data, error } = await supabase
      .from("profiles")
      .select("id, name, email, role, permissions, is_active, created_at")
...
// Lista mínima de perfis ativos para dropdowns de responsáveis (imersões).
// Não depende da permissão de "Usuários" no frontend.
export async function listAssignableProfiles() {
  ...
  const r = await fetch("/api/profiles/assignable", {
    method: "GET",
    headers: { Authorization: `Bearer ${token}` }
  });
  if (!r.ok) {
