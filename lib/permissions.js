...
export function normalizeRole(role) {
  // Normaliza roles vindos do banco/UI.
  // Importante: em alguns fluxos o role pode vir capitalizado (ex: "Consultor").
  // Padronizamos tudo para minúsculo para garantir consistência em filtros/permissões.
  const r = String(role || "").trim().toLowerCase();
  if (!r) return "viewer";
  if (r === "consultor_educacao" || r === "consultor educacao" || r === "consultor-educacao") return "consultor";
  return r;
}
...
export function canSeeMenuItem(role, itemKey) {
  // itemKey: dashboard | immersoes | painel | relatorios | templates | cadastro_imersoes | notificacoes_email | palestrantes | usuarios
  const r = normalizeRole(role);
