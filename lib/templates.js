...
export async function listTemplateItems(_templateId) {
  ...
  try {
    ...
    // Compat: algumas bases antigas podem não ter a coluna `area`.
    // Fazemos fallback sem quebrar a UI.
    ...
    const { data, error } = await baseQuery.select(
      "id, template_id, phase, area, title, due_basis, offset_days, sort_order, created_at"
    );
    if (!error) return data ?? [];

    const msg = String(error?.message || "");
    const isMissingArea = msg.includes("checklist_template_items.area") || msg.includes("column \"area\" does not exist");
    if (!isMissingArea) throw error;

    const { data: data2, error: error2 } = await baseQuery.select(
      "id, template_id, phase, title, due_basis, offset_days, sort_order, created_at"
    );
    if (error2) throw error2;
    return (data2 ?? []).map((r) => ({ ...r, area: null }));
  }
}
...
export async function createTemplateItem(_payload) {
  ...
  try {
    ...
    delete payload2.area;
    const { error: error2 } = await supabase.from("checklist_template_items").insert(payload2);
    if (error2) throw error2;
  }
}
...
export async function updateTemplateItem(_id, _payload) {
  ...
  try {
    ...
    const payload2 = { ...payload };
    delete payload2.area;
    const { error: error2 } = await supabase.from("checklist_template_items").update(payload2).eq("id", id);
    if (error2) throw error2;
  }
}
