...
export async function updateTask(taskId, patch) {
  ...
  if (!taskId) throw new Error("Task ID ausente para atualização.");
  ...
  // Observação Supabase/PostgREST:
  // Em ambientes com RLS/policies ou preferências de retorno, o UPDATE pode retornar 0 linhas.
  // Usar .single() dispara "Cannot coerce the result to a single JSON object". Aqui tratamos esse caso.
  ...
  // Fallback: busca o registro atualizado. Se a policy bloquear SELECT, retornamos o patch mínimo
  // para que a UI consiga refletir o estado sem quebrar o fluxo.
  const { data: fetched, error: eSel } = await supabase
    .from("immersion_tasks")
    .select("*")
    .eq("id", taskId)
    .maybeSingle();
  if (!eSel && fetched) return fetched;
