...
export async function listProfiles({ onlyActive = false } = {}) {
  ensure();

  // IMPORTANTE:
  // A tabela `profiles` normalmente possui RLS. Perfis que não são "admin"
  // podem não ter permissão para dar SELECT direto na tabela, o que quebra
  // selects (ex.: criação de imersão) com listas vazias.
  //
  // Para resolver isso de forma determinística, usamos um endpoint server-side
  // que lê via Service Role e aplica filtros/segurança por token.
  ...
  const resp = await fetch(`/api/profiles/list?${params.toString()}`, {
    method: "GET",
    headers: {
  ...
  if (!resp.ok) {
    const msg = payload?.error || payload?.message || "Falha ao listar usuários.";
    throw new Error(msg);
  }
...
export async function listActiveProfiles() {
  return listProfiles({ onlyActive: true });
}
...
export async function setUserPassword(id, password) {
  return await adminFetch("/api/admin/set-user-password", { method: "POST", body: { id, password } });
}
