...
export async function getDashboardStats() {
  ...
  const workload = Array.from(byResp.values())
    .map((w) => {
      // score: overdue pesa mais, depois vencendo em 3 dias, depois volume aberto
      const score = (w.overdue * 5) + (w.dueSoon * 3) + Math.min(10, w.open);
      let level = "Baixo";
      if (score >= 18 || w.overdue >= 5) level = "Alto";
      else if (score >= 10 || w.overdue >= 2 || w.open >= 10) level = "Médio";
      const reasons = [];
      if (w.overdue) reasons.push(`${w.overdue} atrasada(s)`);
      if (w.dueSoon) reasons.push(`${w.dueSoon} vence(m) em até ${dueSoonDays} dias`);
      if (w.open >= 10) reasons.push(`${w.open} abertas`);
      return { ...w, score, level, reasons };
    })
    .sort((a, b) => (b.score - a.score) || (b.open - a.open) || (b.overdue - a.overdue))
    .slice(0, 10);
