...
// Status helpers (robustos para variações de texto/acento e bases legadas)
export function normalizeTaskStatus(status) {
  const s = (status || "").toString().trim().toLowerCase();
  if (!s) return "";
  // remove acentos
  const noAccent = s.normalize("NFD").replace(/\p{M}/gu, "");
  if (noAccent === "concluida" || noAccent === "concluido" || noAccent === "done" || noAccent === "finalizada") return "done";
  if (noAccent === "cancelada" || noAccent === "cancelado" || noAccent === "canceled") return "canceled";
  if (noAccent === "em andamento" || noAccent === "andamento" || noAccent === "doing") return "doing";
  if (noAccent === "planejamento" || noAccent === "a fazer" || noAccent === "aberta" || noAccent === "open" || noAccent === "todo") return "open";
  return noAccent;
}
...
function computeDefaultsFromImmersion({ immersion, phase }) {
  ...
  // Regra do produto: o dono de cada tarefa é o Consultor cadastrado na imersão.
  // Mantemos fallback para checklist_owner_id apenas para compatibilidade com bases antigas.
  const responsible_id = immersion?.educational_consultant || immersion?.checklist_owner_id || null;
...
async function enrichTaskPayload(payload) {
  ...
  // Defensivo: alguns flows antigos (templates/imports/testes) podem enviar phase/status nulos.
  // O banco pode ter NOT NULL em `phase`, então garantimos um default previsível.
  if (!clean.phase) clean.phase = "PA-PRE";
  if (!clean.status) clean.status = "Programada";
  const immersionId = clean.immersion_id;
  if (!immersionId) return clean;

  // Regra do produto: responsável sempre deve ser o Consultor da imersão.
  // Portanto, buscamos a imersão para (a) aplicar o responsável e (b) calcular prazo quando faltar.
  ...
  return {
    ...clean,
    // Override (se houver consultor configurado). Se não houver, mantém o valor recebido.
...
export async function createTasks(payloads) {
  ...
  for (const [immersionId, items] of groups.entries()) {
    ...
    // Regra do produto: responsável padrão é o Dono da imersão.
    // Para garantir consistência, buscamos a imersão (1x por grupo) e aplicamos override (fallback: consultor).
    const immersion = await getImmersion(immersionId);
...
export async function updateTask(taskId, patch) {
  ...
  if (!taskId) throw new Error("taskId é obrigatório para atualizar a tarefa.");
  const clean = sanitizeTaskPayload(patch || {});

  // Conclusão no produto é inferida por `done_at` (e algumas bases legadas usam `completed_at`).
  // Se o usuário alterou o status, garantimos consistência desses campos.
  if ("status" in clean) {
    const normalized = normalizeTaskStatus(clean.status);
    const nowIso = new Date().toISOString();
    if (normalized === "done") {
      // compatibilidade: alguns pontos do app usam done_at; outros, completed_at
      if (!("done_at" in clean)) clean.done_at = nowIso;
      if (!("completed_at" in clean)) clean.completed_at = nowIso;
    } else {
      // Reabrir: limpa marcadores de conclusão
      if (!("done_at" in clean)) clean.done_at = null;
      if (!("completed_at" in clean)) clean.completed_at = null;
    }
  }

  // Regra: tarefa concluída não pode ter responsável alterado.
  // Defensivo: como o app atualiza direto via Supabase, reforçamos aqui no client.
  if ("responsible_id" in clean) {
    ...
    if (isTaskDone(current)) {
      delete clean.responsible_id;
    }
  }
  // IMPORTANTE (Supabase/PostgREST): em tabelas com RLS, `update(...).select().single()` pode falhar com
  // 406 (Not Acceptable) quando nenhuma linha pode ser retornada. Por isso, executamos o UPDATE sem
  // pedir representação, e fazemos um SELECT separado para recarregar o estado.
  ...
  const { data: refreshed, error: eSel } = await supabase
    .from("immersion_tasks")
    .select("*")
    .eq("id", taskId)
    .maybeSingle();
  if (eSel) throw eSel;
  return refreshed ?? { id: taskId, ...clean };
}
...
export async function bulkUpdateTasks(taskIds, patch) {
  ...
  // Regra: tarefa concluída não pode ter responsável alterado.
  // Se vier responsible_id no patch, aplicamos somente nas tarefas NÃO concluídas.
  if ("responsible_id" in clean) {
    ...
    const notDoneIds = (rows || []).filter((t) => !isTaskDone(t)).map((t) => t.id);
    ...
    // Retorna estado final (inclui concluídas sem alteração) para manter UI consistente.
    const { data: finalRows, error: eFinal } = await supabase
      .from("immersion_tasks")
      .select("*")
      .in("id", ids);
