...
import { getImmersion, updateImmersion, listImmersionCatalog } from "../../lib/immersions";
...
export default function EditarImersaoPage() {
  ...
  // Carrega a imersão existente para edição (mantém o layout idêntico ao da tela "Editar imersão").
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setForm((p) => ({
          ...p,
          immersion_catalog_id: data?.immersion_catalog_id ? String(data.immersion_catalog_id) : p.immersion_catalog_id,
          immersion_name: data?.immersion_name ?? data?.name ?? p.immersion_name,
          type: data?.type ?? data?.immersion_type ?? data?.format ?? p.type,
          start_date: data?.start_date ?? p.start_date,
          end_date: data?.end_date ?? p.end_date,
          room_location: data?.room_location ?? data?.room ?? p.room_location,
          status: data?.status ?? p.status,
          educational_consultant: data?.educational_consultant ?? p.educational_consultant,
          checklist_owner_id: data?.checklist_owner_id ?? p.checklist_owner_id,
          instructional_designer: data?.instructional_designer ?? p.instructional_designer,
          production_responsible: data?.production_responsible ?? p.production_responsible,
          events_responsible: data?.events_responsible ?? p.events_responsible,

          trainer_speaker_id: data?.trainer_speaker_id ?? p.trainer_speaker_id,
          speaker_ids: Array.isArray(data?.speaker_ids) ? data.speaker_ids : p.speaker_ids,
          checklist_template_id: data?.checklist_template_id ?? p.checklist_template_id,
          mentors_present: data?.mentors_present ?? p.mentors_present,
          need_specific_staff: !!(data?.need_specific_staff ?? p.need_specific_staff),
          staff_justification: data?.staff_justification ?? p.staff_justification,
          service_order_link: data?.service_order_link ?? p.service_order_link,
          technical_sheet_link: data?.technical_sheet_link ?? p.technical_sheet_link,
        }));
      } catch (e) {
        setError(e?.message || "Erro ao carregar a imersão.");
      } finally {
  ...
  // Best-effort: se a imersão antiga não tem immersion_catalog_id, tenta inferir pelo par (nome + formato).
  useEffect(() => {
    if (!immersionCatalog?.length) return;
    setForm((p) => {
      if (p.immersion_catalog_id) return p;
      if (!p.immersion_name || !p.type) return p;

      const nameKey = String(p.immersion_name).trim().toLowerCase();
      const typeKey = normalizeFormatValue(p.type);

      const found = immersionCatalog.find((c) => {
        const cName = String(c?.name || "").trim().toLowerCase();
        const cType = normalizeFormatValue(c?.format);
        return cName === nameKey && cType === typeKey;
      });

      if (!found?.id) return p;
      return { ...p, immersion_catalog_id: String(found.id) };
    });
  }, [immersionCatalog]);
...
useEffect(() => {
  ...
  async function onSubmit(e) {
    ...
    setSaving(true);
    try {
      await updateImmersion(id, {
        immersion_catalog_id: form.immersion_catalog_id || null,
      ...
      setToast({ kind: "success", message: "Imersão atualizada." });
      router.push(`/imersoes/${id}`);
} catch (e) {
          ...
          {tab !== "informacoes" ? (
            <div className="tabEmpty" role="status" aria-live="polite">
              <div className="small muted" style={{ marginBottom: 10 }}>
                As demais abas ficam disponíveis após criar a imersão. Complete as informações e clique em “Editar imersão”.
              </div>
      ...
      <Field label="Formato" hint="Obrigatório">
        <select className="input" value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))}>
          ...
          <div className="row" style={{ justifyContent: "space-between" }}> 
            ...
            <button className="btn primary" type="submit" disabled={saving}>{saving ? "Criando..." : "Editar imersão"}</button>
          </div>
        </form>
      </div>
    </Layout>
