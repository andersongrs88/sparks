import { supabase } from "./supabaseClient";
import { getImmersion } from "./immersions";

// Evita erros quando o banco tem colunas diferentes (bases antigas/novas)
const TASK_COLUMNS_ALLOWLIST = new Set([
  "immersion_id",
  "phase",
  "area",
  "title",
  "responsible_id",
  "due_date",
  "status",
  "notes",
  // Auditoria (opcional; ignora se não existir no schema)
  "created_by",
  "completed_by",
  "completed_at",
  "evidence_link",
  "evidence_path",
  // Compat
  "done_at",
]);

// Status helpers (robustos para variações de texto/acento e bases legadas)
export function normalizeTaskStatus(status) {
  const s = (status || "").toString().trim().toLowerCase();
  if (!s) return "";
  // remove acentos
  const noAccent = s.normalize("NFD").replace(/\p{M}/gu, "");
  if (noAccent === "concluida" || noAccent === "concluido" || noAccent === "done" || noAccent === "finalizada") return "done";
  if (noAccent === "cancelada" || noAccent === "cancelado" || noAccent === "canceled") return "canceled";
  if (noAccent === "em andamento" || noAccent === "andamento" || noAccent === "doing") return "doing";
  if (noAccent === "planejamento" || noAccent === "a fazer" || noAccent === "aberta" || noAccent === "open" || noAccent === "todo") return "open";
  return noAccent;
}

export function isTaskDone(task) {
  if (!task) return false;
  if (task.done_at) return true;
  const st = normalizeTaskStatus(task.status);
  return st === "done";
}

export function isTaskCanceled(task) {
  const st = normalizeTaskStatus(task?.status);
  return st === "canceled";
}

function sanitizeTaskPayload(payload) {
  const out = {};
  Object.keys(payload || {}).forEach((k) => {
    if (TASK_COLUMNS_ALLOWLIST.has(k) && payload[k] !== undefined) out[k] = payload[k];
  });
  return out;
}

function addDays(dateISO, days) {
  if (!dateISO) return null;
  const d = new Date(dateISO + "T00:00:00");
  d.setDate(d.getDate() + days);
  return d.toISOString().slice(0, 10);
}

function computeDefaultsFromImmersion({ immersion, phase }) {
  const ph = (phase || "PA-PRE").toString().trim();

  // Responsável automático
  // Regra do produto: o dono de cada tarefa é o Consultor cadastrado na imersão.
  // Mantemos fallback para checklist_owner_id apenas para compatibilidade com bases antigas.
  const responsible_id = immersion?.educational_consultant || immersion?.checklist_owner_id || null;

  // Prazo inteligente (heurística simples e previsível)
  // PA-PRE: -10 dias do início
  // DURANTE: início
  // POS: +3 dias do fim
  let due_date = null;
  if (immersion?.start_date && (ph === "PA-PRE" || ph === "DURANTE")) {
    due_date = ph === "PA-PRE" ? addDays(immersion.start_date, -10) : immersion.start_date;
  }
  if (immersion?.end_date && ph === "POS") {
    due_date = addDays(immersion.end_date, 3);
  }

  return { responsible_id, due_date };
}

async function enrichTaskPayload(payload) {
  const clean = sanitizeTaskPayload(payload || {});

  // Defensivo: alguns flows antigos (templates/imports/testes) podem enviar phase/status nulos.
  // O banco pode ter NOT NULL em `phase`, então garantimos um default previsível.
  if (!clean.phase) clean.phase = "PA-PRE";
  if (!clean.status) clean.status = "Programada";
  const immersionId = clean.immersion_id;
  if (!immersionId) return clean;

  // Regra do produto: responsável sempre deve ser o Consultor da imersão.
  // Portanto, buscamos a imersão para (a) aplicar o responsável e (b) calcular prazo quando faltar.
  const needsDue = !clean.due_date;

  const immersion = await getImmersion(immersionId);
  const { responsible_id, due_date } = computeDefaultsFromImmersion({ immersion, phase: clean.phase });

  return {
    ...clean,
    // Override (se houver consultor configurado). Se não houver, mantém o valor recebido.
    ...(responsible_id ? { responsible_id } : null),
    ...(needsDue ? { due_date } : null),
  };
}

function ensure() {
  if (!supabase) throw new Error("Supabase não configurado (verifique as variáveis do deploy na Vercel).");
}

export async function listTasksByImmersion(immersionId) {
  ensure();
  const { data, error } = await supabase
    .from("immersion_tasks")
    .select("*")
    .eq("immersion_id", immersionId)
    .order("due_date", { ascending: true, nullsFirst: false })
    .order("created_at", { ascending: true });

  if (error) throw error;
  return data ?? [];
}

export async function createTask(payload) {
  ensure();
  const enriched = await enrichTaskPayload(payload);
  const { data, error } = await supabase
    .from("immersion_tasks")
    .insert([enriched])
    .select("*")
    .single();

  if (error) throw error;
  return data;
}

// Marca tarefas como "Atrasada" quando passaram do prazo e ainda não foram concluídas.
// Best-effort: se o schema não aceitar, falha silenciosamente no caller.
export async function syncOverdueTasksForImmersion(immersionId) {
  ensure();
  if (!immersionId) return 0;
  const today = new Date();
  const t0 = new Date(today.getFullYear(), today.getMonth(), today.getDate());
  const todayISO = t0.toISOString().slice(0, 10);

  // Atualiza apenas tarefas ainda abertas
  const { data, error } = await supabase
    .from("immersion_tasks")
    .update({ status: "Atrasada" })
    .eq("immersion_id", immersionId)
    .neq("status", "Concluída")
    .neq("status", "Atrasada")
    .not("due_date", "is", null)
    .lt("due_date", todayISO)
    .select("id");

  if (error) throw error;
  return (data || []).length;
}

export async function syncOverdueTasksGlobal(limitImmersionIds = null) {
  ensure();
  const today = new Date();
  const t0 = new Date(today.getFullYear(), today.getMonth(), today.getDate());
  const todayISO = t0.toISOString().slice(0, 10);

  let q = supabase
    .from("immersion_tasks")
    .update({ status: "Atrasada" })
    .neq("status", "Concluída")
    .neq("status", "Atrasada")
    .not("due_date", "is", null)
    .lt("due_date", todayISO);

  if (Array.isArray(limitImmersionIds) && limitImmersionIds.length > 0) {
    q = q.in("immersion_id", limitImmersionIds);
  }

  const { data, error } = await q.select("id");
  if (error) throw error;
  return (data || []).length;
}

export async function createTasks(payloads) {
  ensure();
  if (!payloads || payloads.length === 0) return [];
  // Enriquecimento em lote (agrupa por imersão para reduzir chamadas)
  const groups = new Map();
  for (const p of payloads) {
    const clean = sanitizeTaskPayload(p || {});
    const key = clean.immersion_id || "__no_immersion__";
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(clean);
  }

  const enrichedAll = [];
  for (const [immersionId, items] of groups.entries()) {
    if (immersionId === "__no_immersion__") {
      enrichedAll.push(...items);
      continue;
    }
    // Regra do produto: responsável padrão é o Dono da imersão.
    // Para garantir consistência, buscamos a imersão (1x por grupo) e aplicamos override (fallback: consultor).
    const immersion = await getImmersion(immersionId);

    for (const it of items) {
      const needsDue = !it.due_date;
      const { responsible_id, due_date } = computeDefaultsFromImmersion({ immersion, phase: it.phase });
      enrichedAll.push({
        ...it,
        // Override (se houver consultor configurado). Se não houver, mantém o valor recebido.
        ...(responsible_id ? { responsible_id } : null),
        ...(needsDue ? { due_date } : null),
      });
    }
  }

  const { data, error } = await supabase.from("immersion_tasks").insert(enrichedAll).select("*");
  if (error) throw error;
  return data ?? [];
}

export async function updateTask(taskId, patch) {
  ensure();
  if (!taskId) throw new Error("updateTask: taskId é obrigatório");
  const clean = sanitizeTaskPayload(patch || {});

  // Regra de produto: concluir deve marcar done_at (algumas telas/listas usam done_at como fonte de verdade).
  // Também mantemos compatibilidade com completed_at quando existir no schema.
  const normalized = normalizeTaskStatus(clean.status);
  if ("status" in clean) {
    if (normalized === "done") {
      const now = new Date().toISOString();
      if (!("done_at" in clean)) clean.done_at = now;
      if (!("completed_at" in clean)) clean.completed_at = now;
    } else {
      // Reabrir: se o usuário mudou o status para não-concluída, limpamos marcadores de conclusão.
      if ("done_at" in clean) {
        // se veio explicitamente, respeita
      } else {
        clean.done_at = null;
      }
      if ("completed_at" in clean) {
        // se veio explicitamente, respeita
      } else {
        clean.completed_at = null;
      }
    }
  }

  // Regra: tarefa concluída não pode ter responsável alterado.
  // Defensivo: como o app atualiza direto via Supabase, reforçamos aqui no client.
  if ("responsible_id" in clean) {
    const { data: current, error: eCur } = await supabase
      .from("immersion_tasks")
      .select("id, status, done_at")
      .eq("id", taskId)
      .maybeSingle();
    if (eCur) throw eCur;
    if (isTaskDone(current)) {
      delete clean.responsible_id;
    }
  }
  const { data, error } = await supabase
    .from("immersion_tasks")
    .update({ ...clean, updated_at: new Date().toISOString() })
    .eq("id", taskId)
    .select("*")
    .maybeSingle();

  // PostgREST pode retornar 0 linhas (ex.: RLS + prefer de retorno). Evita quebra da UI.
  if (error) throw error;

  if (data) return data;

  // Fallback: busca o registro atualizado (se for permitido pela policy)
  const { data: fresh, error: eFresh } = await supabase
    .from("immersion_tasks")
    .select("*")
    .eq("id", taskId)
    .maybeSingle();

  if (eFresh) throw eFresh;
  if (fresh) return fresh;

  // Último recurso: retorna um objeto mínimo para não quebrar o estado local
  return { id: taskId, ...clean };
}

export async function bulkUpdateTasks(taskIds, patch) {
  ensure();
  const ids = (taskIds || []).filter(Boolean);
  if (!ids.length) return [];
  const clean = sanitizeTaskPayload(patch || {});

  // Regra: tarefa concluída não pode ter responsável alterado.
  // Se vier responsible_id no patch, aplicamos somente nas tarefas NÃO concluídas.
  if ("responsible_id" in clean) {
    const { data: rows, error: eRows } = await supabase
      .from("immersion_tasks")
      .select("id, status, done_at")
      .in("id", ids);
    if (eRows) throw eRows;

    const notDoneIds = (rows || []).filter((t) => !isTaskDone(t)).map((t) => t.id);

    if (notDoneIds.length) {
      const { error: eUpd } = await supabase
        .from("immersion_tasks")
        .update({ ...clean, updated_at: new Date().toISOString() })
        .in("id", notDoneIds);
      if (eUpd) throw eUpd;
    }

    // Retorna estado final (inclui concluídas sem alteração) para manter UI consistente.
    const { data: finalRows, error: eFinal } = await supabase
      .from("immersion_tasks")
      .select("*")
      .in("id", ids);
    if (eFinal) throw eFinal;
    return finalRows || [];
  }
  const { data, error } = await supabase
    .from("immersion_tasks")
    .update({ ...clean, updated_at: new Date().toISOString() })
    .in("id", ids)
    .select("*");

  if (error) throw error;
  return data || [];
}


export async function deleteTask(taskId) {
  ensure();
  const { error } = await supabase.from("immersion_tasks").delete().eq("id", taskId);
  if (error) throw error;
  return true;
}

// ----------------------------
// Helpers
// ----------------------------
export function sortTasksByPriority(tasks) {
  const toTime = (d) => (d ? new Date(d).getTime() : null);

  return (tasks || []).slice().sort((a, b) => {
    const aDone = isTaskDone(a);
    const bDone = isTaskDone(b);
    if (aDone !== bDone) return aDone ? 1 : -1;

    const aDue = toTime(a?.due_date);
    const bDue = toTime(b?.due_date);
    if (aDue !== null && bDue !== null && aDue !== bDue) return aDue - bDue;
    if (aDue === null && bDue !== null) return 1;
    if (aDue !== null && bDue === null) return -1;

    const aUpd = toTime(a?.updated_at);
    const bUpd = toTime(b?.updated_at);
    if (aUpd !== null && bUpd !== null && aUpd !== bUpd) return bUpd - aUpd;

    const aCr = toTime(a?.created_at) || 0;
    const bCr = toTime(b?.created_at) || 0;
    return aCr - bCr;
  });
}
