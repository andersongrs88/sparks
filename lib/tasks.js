...
export async function updateTask(taskId, patch) {
  ...
  if (!taskId) throw new Error("ID da tarefa ausente.");
  ...
  // Observação (RLS/PostgREST): dependendo das policies, um UPDATE pode não retornar linhas
  // mesmo tendo sido aplicado com sucesso. Portanto, usamos maybeSingle() e um fallback de SELECT.
  const { data, error } = await supabase
    .from("immersion_tasks")
  ...
  // Fallback: tenta buscar o registro (se o SELECT for permitido). Se ainda assim vier vazio,
  // retornamos um objeto mínimo para a UI continuar.
  const { data: fetched, error: eFetch } = await supabase
    .from("immersion_tasks")
    .select("*")
    .eq("id", taskId)
    .maybeSingle();
