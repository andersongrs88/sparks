...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const payload = {
      ...
      is_internal: is_internal === false ? false : true,
      vignette_name: String(vignette_name || "").trim() || null,
    };
    const { data, error } = await admin.from("speakers").insert(payload).select("id").single();
    if (error) return json(res, 400, { error: error.message });
