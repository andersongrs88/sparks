...
export async function listActiveProfiles() {
  // Prefer a protected API route that can list users regardless of RLS.
  // This is required for non-admin users (ex: Consultor) to populate dropdowns.
  try {
    const data = await adminFetch("/api/users/list");
    const users = Array.isArray(data) ? data : data?.users;
    if (Array.isArray(users)) {
      return users.filter((p) => !!p.is_active);
    }
  } catch (_) {
    // fallback below
  }
