...
export async function listNotificationTasks({ user, immersionId = null }) {
  ensure();
  if (!user) return [];
  ...
  let q = supabase
    ...
    .select("id, immersion_id, phase, title, responsible_id, due_date, status, evidence_link, evidence_path, created_at, immersions(immersion_name)")
