...
export async function updateTask(taskId, patch) {
  ...
  if (!taskId) throw new Error("taskId ausente ao atualizar tarefa");
  ...
  // PATCH sem retorno (evita 406 com RLS/PostgREST quando Prefer: return=representation entra em conflito).
  const { error: eUpd } = await supabase
    .from("immersion_tasks")
    .update({ ...clean, updated_at: nowIso })
    .eq("id", taskId);
  if (eUpd) throw eUpd;

  // Recarrega a linha em uma chamada separada (SELECT costuma ser permitido mesmo quando UPDATE não pode retornar a linha).
  const { data, error: eSel } = await supabase
    ...
    .maybeSingle();
  if (eSel) throw eSel;
  return data || { id: taskId, ...clean, updated_at: nowIso };
}
