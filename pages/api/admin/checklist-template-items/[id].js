...
export default async function handler(req, res) {
  ...
  if (req.method === "PATCH") {
    ...
    const patch = {};
    if (phase !== undefined) patch.phase = String(phase || "").trim() || null;
    if (area !== undefined) patch.area = String(area || "").trim() || null;
    if (title !== undefined) {
      const t = String(title || "").trim();
      if (!t) return json(res, 400, { error: "Título é obrigatório." });
      patch.title = t;
    }
    if (due_basis !== undefined) patch.due_basis = String(due_basis || "start").trim() === "end" ? "end" : "start";
    if (offset_days !== undefined) patch.offset_days = Number(offset_days ?? 0);
    if (sort_order !== undefined) patch.sort_order = Number(sort_order ?? 0);

    const { error } = await admin.from("checklist_template_items").update(patch).eq("id", id);
