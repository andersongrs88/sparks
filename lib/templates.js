...
export async function listTemplateItems(_templateId) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id, template_id, phase, area, responsible_id, title, due_basis, offset_days, sort_order, created_at")
...
export async function createTemplateItem(_payload) {
  const payload = {
    ...
    responsible_id: _payload?.responsible_id ? String(_payload.responsible_id).trim() : null,
